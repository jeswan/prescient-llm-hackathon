...
class GPT(nn.Module):
    def __init__(self, config: Config = None) -> None:
        ...
        self.transformer = nn.ModuleDict(
            dict(
                ...
                vit=ViT(),
                linear=nn.Linear(in_features=768, out_features=config.n_embd),
    ...
    def forward(
        self, idx: torch.Tensor, img: torch.Tensor, max_seq_length: Optional[int] = None, input_pos: Optional[torch.Tensor] = None
    ) -> torch.Tensor:
...
class ViT(nn.Module):
    def __init__(self) -> None:
        ...
        self.patch_embeddings = self.model.get_input_embeddings()
